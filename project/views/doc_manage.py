    ...
    try:
        ...
        if dir_name == 'root':
            ...
            if Document.objects.filter(project=project, name=name, par_dir=sub_dir.first()).exists():
                return ResponseTemplate(Error.DOC_EXISTS, 'Document already exists')
            document = Document.objects.create(project=project, name=name, par_dir=sub_dir.first())
        DocumentContent.objects.create(document=document)  # content is json format
        return ResponseTemplate(Error.SUCCESS, 'create new document successfully', data=DocumentSerializer(document).data)
